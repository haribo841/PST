# Compute alternative system capacities for two primary systems (C1=3, C2=5)
# and produce plots for Bj = [0.2, 0.4, 0.6], Aalt = 3 Erl, a in [0.8,1.7] step 0.1.
import math
import numpy as np
...
# Parameters
Ci = np.array([3, 5], dtype=int)  # C1=3, C2=5
Aalt = 3.0
Bj_list = [0.2, 0.4, 0.6]
amin = 0.8
amax = 1.7
...
for a in a_values:
    # offered traffic to each primary group (a * Ci)
    ...
    for Bj in Bj_list:
        # EqChange: total mean to alternative -> find minimal Calt with ErlangB <= Bj
        Calt_eq = 0
        if Mean_alt_total <= 0:
            Calt_eq = 0
        else:
...
txt_path = out_dir / "wyniki_C1_3_C2_5_Aalt3_range_a_0.8_1.7.txt"
csv_path = out_dir / "wyniki_C1_3_C2_5_Aalt3_range_a_0.8_1.7.csv"
df.to_csv(csv_path, index=False, sep=';')
with open(txt_path, "w", encoding="utf-8") as f:
    f.write(df.to_string(index=False))

# Create plots per Bj and combined
plot_paths = []
for Bj in Bj_list:
    ...
    plt.title(f'Porównanie metod (C1=3, C2=5, Aalt={Aalt}, B={Bj})')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    pfile = out_dir / f'porownanie_C1_3_C2_5_Aalt3_B{int(Bj*100)}.png'
...
plt.xlabel('a (ruch przypadający na 1 jednostkę pojemności)')
plt.ylabel('Wymagana pojemność systemu alternatywnego Calt')
plt.title(f'Porównanie metod (C1=3, C2=5, Aalt={Aalt})')
plt.grid(True)
plt.legend(ncol=2)
plt.tight_layout()
combined_path = out_dir / 'porownanie_C1_3_C2_5_Aalt3_all_B.png'
plt.savefig(combined_path, dpi=200)
plt.show()
plot_paths.append(str(combined_path))

plot_paths, str(txt_path), str(csv_path)
